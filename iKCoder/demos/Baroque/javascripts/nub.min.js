...
var EASE_ORBIT_GRAB = .2;

var EASE_ORBIT_FOLLOW = .02;

var EASE_ORBIT_RESTORE = .02;

var EASE_CENTER_GRAB = .3;

var EASE_CENTER_RESTORE = .015;

var EASE_CENTER_FOLLOW_MIN = .04;

var EASE_CENTER_FOLLOW_MAX = .01;

var EASE_ORBIT_LOADER = .003;

var EASE_CENTER_LOADER = .03;

var EASE_ORBIT_EXIT_LOADER = .1;

var EASE_CENTER_EXIT_LOADER = .1;

var TRAIL_OPAC_MIN = .04;

var TRAIL_OPAC_MAX = .5;
...
var ORBIT_FOLLOW_MIN = WHEEL_RADIUS * .3;

var ORBIT_FOLLOW_MAX = WHEEL_RADIUS * .5;
...
Nub.prototype.updPos = function () {
    ...
    if (this.isTossing) {
        ...
        var b = this.isMouseOver ? 0 : this.orbitTarg;
        if (this.orbit != b) {
            this.orbit += (b - this.orbit) * this.easeOrbit;
        ...
        if (this.isFirstRun) {
            ...
            for (var a = 0; a < TRAIL_PTS; a++) {
                this.arrTrail[a] = new Point(this.xpw, this.ypw);
            }
        }
    ...
    for (var a = 0; a < this.arrTrail.length; a++) {
        this.m.checkBoxLimit(this.arrTrail[a].x, this.arrTrail[a].y);
    }
    this.frameCt++;
};
...
Nub.prototype.enter = function () {
    ...
    if (this.indAll == 0) {
        this.xpOrbit = -150;
        this.ypOrbit = this.m.height * .6;
        this.orbit = ORBIT_LOADER * .3;
    } else {
        if (this.indAll == 1) {
            this.xpOrbit = this.m.width * 1;
            this.ypOrbit = -this.m.height * .9;
...
Nub.prototype.grab = function () {
    ...
    this.isGrabbed = true;
    this.radTarg = RAD_GRAB;
    this.orbitTarg = ORBIT_GRAB;
    var b = Math.floor(Math.random() * NUBS);
    if (b == NUBS) {
        b = NUBS - 1;
    }
    for (var a = 0; a < NUBS; a++) {
        var c = this.machine.arrNubs[b];
        if (c != this) {
            c.follow(a);
        }
        b = (b + 1) % NUBS;
...
Nub.prototype.drop = function () {
    ...
    this.velX = this.m.dx;
    this.velY = this.m.dy;
    var a = Math.abs(this.velX);
    var b = Math.abs(this.velY);
    if (a > THROW_SPD_MAX) {
        this.velX = a / this.velX * THROW_SPD_MAX;
    } else {
        if (a < THROW_SPD_MIN) {
            if (this.dx == 0) {
                this.velX = (Math.random() > .5 ? -1 : 1) * THROW_SPD_MIN;
            } else {
                this.velX = Math.abs(this.dx) / this.dx * THROW_SPD_MIN;
            }
        }
    }
    if (b > THROW_SPD_MAX) {
        this.velY = b / this.velY * THROW_SPD_MAX;
    } else {
        if (b < THROW_SPD_MIN) {
            if (this.dy == 0) {
                this.velY = (Math.random() > .5 ? -1 : 1) * THROW_SPD_MIN;
            } else {
                this.velY = Math.abs(this.dy) / this.dy * THROW_SPD_MIN;
            }
        }
    }
    ...
    var e = 1;
    var c;
    for (var d = 0; d < NUBS; d++) {
        c = this.machine.arrNubs[d];
        if (c != this && c.hasEntered) {
            c.unfollow(this, this.velX, this.velY, e / NUBS);
        }
        e++;
    }
};

Nub.prototype.follow = function (a) {
    var b = a / (NUBS - 1);
    this.orbitTarg = lerp(ORBIT_FOLLOW_MIN, ORBIT_FOLLOW_MAX, b);
    this.easeCenterFollow = lerp(EASE_CENTER_FOLLOW_MIN, EASE_CENTER_FOLLOW_MAX, b);
    this.easeOrbit = EASE_ORBIT_FOLLOW;
    this.easeCenter = this.easeCenterFollow;
};

Nub.prototype.unfollow = function (g, e, d, a) {
    var c = g.xp1 - this.xp1;
    var b = g.yp1 - this.yp1;
    var f = .05;
    this.velX = (c * f + e * (1 - f)) * a;
    this.velY = (b * f + d * (1 - f)) * a;
    this.orbitTarg = WHEEL_RADIUS;
    this.easeOrbit = EASE_ORBIT_RESTORE;
    this.easeCenter = EASE_CENTER_RESTORE;
};
...
Nub.prototype.checkMouseOver = function () {
    var b = this.machine.getUserX() - this.xp1;
    var a = this.machine.getUserY() - this.yp1;
    var c = b * b + a * a;
    return c < ROLLOVER_RAD_SQ;
};

Nub.prototype.returnFromBackground = function () {
    for (var a = 0; a < TRAIL_PTS; a++) {
        this.arrTrail[a] = new Point(this.xp0, this.yp0);
    }
};
...
Nub.prototype.redraw = function () {
    ...
    for (var f = 1; f < g; f++) {
        ...
        this.cv.lineWidth = 1;
        if (n < TRAIL_FADEOUT) {
            e = lerp(TRAIL_OPAC_MIN, TRAIL_OPAC_MAX, n / TRAIL_FADEOUT);
        } else {
            e = lerp(TRAIL_OPAC_MAX, TRAIL_OPAC_MIN, (n - TRAIL_FADEOUT) / (1 - TRAIL_FADEOUT));
        }
        this.cv.strokeStyle = "rgba(255,255,255," + e + ")";
        this.cv.moveTo(this.m.xo + l, this.m.yo + a);
        this.cv.quadraticCurveTo(this.m.xo + c, this.m.yo + k, this.m.xo + j, this.m.yo + o);
        d = c;
        m = k;
        c = b;
        k = h;
