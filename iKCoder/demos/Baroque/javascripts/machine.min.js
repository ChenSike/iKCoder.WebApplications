...
var Machine = function (a) {
    ...
    this.xbLimitMin = -MAX_LENGTH * .5;
    this.xbLimitMax = MAX_LENGTH * .5;
    this.ybLimitMin = -HEIGHT_ALL_THREADS * .5;
    this.ybLimitMax = HEIGHT_ALL_THREADS * .5;
...
Machine.prototype.beginLoading = function () {
    this.tFrame0 = this.tSong0 = this.tNotes0 = this.tLoadPrev = this.tLoading0 = this.t0 = new Date().getTime() / 1e3;
    this.ctFrame = 0;
...
Machine.prototype.setGroup = function (c) {
    this.indGroup = c;
...
Machine.prototype.upd = function () {
    ...
    this.updWheels();
    this.cv.clearRect(this.xo + this.xbMin - CLEAR_RECT_MARG, this.yo + this.ybMin - CLEAR_RECT_MARG, this.xbMax - this.xbMin + CLEAR_RECT_MARG * 2, this.ybMax - this.ybMin + CLEAR_RECT_MARG * 2);
    this.updateAndRedrawThreads();
    this.redrawNubs();
};

Machine.prototype.updTime = function () {
    this.t1 = new Date().getTime() / 1e3;
...
Machine.prototype.updWheels = function () {
    var a;
    this.wheel0.setRot(MATH_PI * (.25 + this.beatSong % 16 / 16 * 2) % (2 * MATH_PI));
    this.wheel1.setRot(MATH_PI * (.25 - this.beatSong % 16 / 16 * 2) % (2 * MATH_PI));
    this.wheel0.upd();
    this.wheel1.upd();
};

Machine.prototype.switchBackgroundMode = function (b) {
    switch (b) {
        case 1:
            break;
        ...
        case 0:
            for (var a = 0; a < 4; a++) {
                this.arrNubs[a].returnFromBackground();
            }
            break;
...
Machine.prototype.updFramerate = function () {
    this.tFrame1 = new Date().getTime() / 1e3;
...
Machine.prototype.incrLoad = function () {
    ...
    var d = Math.floor(Math.random() * .999 * k);
    var a, m, b;
    var f = 0;
...
Machine.prototype.updPos = function () {
    ...
    this.isMouseMoving = this.dist > .2;
...
Machine.prototype.updMouseUp = function () {
    if (this.nubOver != null && !this.isHoldingNub) {
        if (this.nubOver.checkMouseOver()) {
            ...
            this.nubOver.rollOut();
            this.nubOver = null;
        }
    }
    for (var a = 0; a < 4; a++) {
        if (this.arrNubs[a].checkMouseOver()) {
            this.nubOver = this.arrNubs[a];
...
Machine.prototype.checkBoxLimit = function (a, b) {
    if (a < this.xbMin) {
        this.xbMin = a;
    } else {
        if (a > this.xbMax) {
